...
const NewsArticleWidget = () => {
  ...
  useEffect(() => {
    fetch('/api/discover?mode=preview')
      .then((res) => res.json())
      .then((data) => {
        // API đã filter thumbnail rồi, không cần filter lại
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm flex flex-row items-stretch w-full h-24 min-h-[96px] max-h-[96px] p-0 overflow-hidden">
      {loading ? (
        <div className="animate-pulse flex flex-row items-stretch w-full h-full">
          <div className="w-24 min-w-24 max-w-24 h-full bg-gray-100" />
        ...
        >
          <div className="relative w-24 min-w-24 max-w-24 h-full overflow-hidden">
            <img
              className="object-cover w-full h-full bg-gray-100 group-hover:scale-110 transition-transform duration-300"
              src={
          ...
          <div className="flex flex-col justify-center flex-1 px-3 py-2">
            <div className="font-semibold text-xs text-black leading-tight line-clamp-2 mb-1">
              {article.title}
            </div>
            <p className="text-black/60 text-[10px] leading-relaxed line-clamp-2">
              {article.content}
            </p>
          </div>
        </a>
